...
function renderCatalog(filter='all'){
  ...
  catalog.replaceChildren(...visible.map((p,i)=>{const article=document.createElement('article');article.className='product-card reveal visible';article.innerHTML=`<div class="card-meta"><span>${p.category}</span><span class="card-number">${String(i+1).padStart(2,'0')}</span></div><button class="product-photo" type="button" aria-label="View ${p.name} details"><img src="${p.image}" alt="Biotin Feeds ${p.name} bag" loading="lazy" decoding="async" width="600" height="800"></button><div class="product-labels"><span class="product-format">${p.type}</span><span class="product-stage">${p.bestFor}</span></div><h3>${p.name}</h3><p>${p.benefit}</p><button class="text-button" type="button" aria-label="View ${p.name} details">View details <span>+</span></button>`;article.querySelectorAll('button').forEach(button=>button.addEventListener('click',()=>openProduct(p)));return article}));
}
renderCatalog();
